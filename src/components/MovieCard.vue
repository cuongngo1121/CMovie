<template>
  ...
  >
    ...
    >
      ...
      <!-- Info Section -->
      <div class="p-1.5 md:p-3 flex-1 flex flex-col bg-theme-card">
        <h3 class="text-[10px] sm:text-xs md:text-sm font-bold text-theme-primary group-hover:text-brand transition-colors line-clamp-2 leading-tight mb-0.5 md:mb-1 min-h-[2.5rem] md:min-h-0">
          {{ movie.name }}
        </h3>
        
        <p class="hidden sm:block text-[10px] md:text-xs text-theme-muted line-clamp-1 mb-1 md:mb-2 opacity-70">
          {{ movie.origin_name }}
        </p>
        
        <div class="mt-auto hidden sm:flex items-center justify-between text-[10px] md:text-xs text-theme-secondary border-t border-theme pt-1.5 md:pt-2">
