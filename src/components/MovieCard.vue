  ...
  >
    ...
    >
      ...
      <div class="relative aspect-[2/3] overflow-hidden bg-gray-200">
        ...
        <!-- Badges -->
        <div class="absolute top-2 left-2 px-2 py-1 bg-brand text-white rounded text-xs font-bold shadow-sm">
          {{ movie.quality || 'HD' }}
        </div>
        
        <div class="absolute top-2 right-2 px-2 py-1 bg-black/60 text-white rounded text-xs font-medium backdrop-blur-sm">
          {{ movie.year }}
        </div>
        
        <div v-if="movie.episode_current && movie.episode_current !== 'Full'" class="absolute bottom-2 left-2 px-2 py-1 bg-blue-600 text-white rounded text-xs font-bold shadow-sm">
          {{ movie.episode_current === 'Hoàn Tất' ? 'FULL' : `TẬP ${movie.episode_current}` }}
        </div>
      </div>
      
      <!-- Info Section -->
      <div class="p-3 flex-1 flex flex-col bg-theme-card">
        <h3 class="text-sm font-bold text-theme-primary group-hover:text-brand transition-colors line-clamp-2 leading-tight mb-1">
          {{ movie.name }}
        </h3>
        ...
        <div class="mt-auto flex items-center justify-between text-xs text-theme-secondary border-t border-theme pt-2">
